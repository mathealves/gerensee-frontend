...
**Feature Branch**: `002-nav-sidebar`  
**Created**: 2026-03-04  
**Status**: Draft  
...
- **SC-005**: The organization name update is reflected in the sidebar, org switcher, and page headings within 500ms of the successful save response — no page reload required.
...
- On first login, the active organization defaults to the first item in the list returned by `GET /organizations`. Active organization state is held in memory only (Zustand in-memory store) and does not survive a page refresh — the user returns to the first org in the list on each new session.
